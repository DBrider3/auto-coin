...
def get_bitcoin_news(
    api_key, query="bitcoin", location="us", language="en", num_results=5
):
    """
    ...
    if "news_results" in results:
        for news_item in results["news_results"][:num_results]:
            news_data.append(
                {"title": news_item.get("title"), "date": news_item.get("date")}
            )
    return news_data
...
def ai_trading():
    ...
    data_payload = {
        "short_term": (
            json.loads(short_term_df.to_json()) if short_term_df is not None else None
        ),
        "mid_term": (
            json.loads(mid_term_df.to_json()) if mid_term_df is not None else None
        ),
        "long_term": (
            json.loads(long_term_df.to_json()) if long_term_df is not None else None
        ),
        "news": news_articles,
    }
