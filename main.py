...
# 페이지 설정
st.set_page_config(
    page_title="Bitcoin AI Trading Dashboard", page_icon="📈", layout="wide"
)
...
def load_trade_data():
    ...
    if len(df) > 0:
        ...
        df["profit_loss_pct"] = (
            df["profit_loss"] / first_trade["portfolio_value"]
        ) * 100
...
if not df.empty and len(df) > 1:
    ...
    # 호버 정보 커스터마이징
    fig.update_traces(hovertemplate="%{x}<br>수익률: %{y:.2f}%<br>")
...
if not df.empty:
    ...
    # 호버 정보 커스터마이징
    fig.update_traces(hovertemplate="%{x}<br>가격: ₩%{y:,.0f}<br>")
...
if not df.empty:
    ...
    with tab2:
        ...
        st.markdown(
            f"### {selected_trade2['timestamp'].strftime('%Y-%m-%d %H:%M')} AI 판단 이유"
        )
